...
import { SaasProjectsPanel } from '../panels/SaasProjectsPanel';
...
const NAV = [
  { id: 'projects', label: 'Projects', icon: FolderOpen, panel: SaasProjectsPanel },
...
export function LeftSidebar({ overlay }: Props) {
  ...
  const {
    ...
    useUiStore();
  const ActivePanel = NAV.find((n) => n.id === activeLeftPanel)?.panel ?? SaasProjectsPanel;
