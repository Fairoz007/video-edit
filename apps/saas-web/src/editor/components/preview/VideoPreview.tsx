...
export function VideoPreview() {
  ...
  const { script, outputPath, status, timeline, input, legacyProjectId, setOutputPath } =
    useProjectStore();
  ...
  const syncOutputFromServer = useCallback(async () => {
    try {
      if (legacyProjectId) {
        const { data } = await getProject(legacyProjectId);
        if (data.outputPath) {
  ...
  }, [legacyProjectId, setOutputPath]);

  useEffect(() => {
    if (!outputPath && (status === 'completed' || legacyProjectId)) {
      syncOutputFromServer();
    }
  }, [outputPath, status, legacyProjectId, syncOutputFromServer]);
