...
import { useConvexProjectSync } from '../../hooks/useConvexProjectSync';

export function EditorLayout({ convexProjectId }: { convexProjectId: Id<'projects'> }) {
  const bp = useBreakpoint();
  const { legacyProjectId, status } = useProjectStore();
  const { mobilePanel, syncLayout, closeMobilePanels } = useUiStore();
  const isMobile = bp === 'mobile';

  useConvexProjectSync(convexProjectId);
  useRenderPolling(legacyProjectId, status === 'rendering');
