...
export function TopToolbar() {
  ...
  const openExport = () => {
    if (outputPath?.startsWith('http')) {
      window.open(outputPath, '_blank', 'noopener,noreferrer');
    } else if (outputPath) {
      window.open(`/api/exports/play/${encodeURIComponent(outputPath.split('/').pop() || '')}`, '_blank');
    } else {
      startRenderFlow();
    }
  };
  ...
  return (
    <header className="h-14 shrink-0 flex items-center gap-3 px-3 sm:px-4 border-b border-forge-border bg-forge-surface/95 backdrop-blur-xl">
      <motion.div className="flex items-center gap-2.5 shrink-0">
        <Link href="/dashboard" className="btn-icon" title="Dashboard">
